...
export default function JobsPreferiti() {
  ...
  return (
    <>
      <Container className="w-75 d-flex justify-content-center mt-2">
        <Card className="compBackgroundForm" style={{ minWidth: "50%" }}>
          <Card.Body className="d-flex flex-column pb-0">
            <div className="d-flex flex-column justify-content-between mb-4">
              <h5>Preferiti</h5>
            </div>
            {preferiti.map((lav, index) => (
              <LavoriPref
                key={index}
                company={lav.company}
                location={lav.location}
                published={lav.published}
                titolo={lav.titolo}
              />
            ))}
          </Card.Body>
