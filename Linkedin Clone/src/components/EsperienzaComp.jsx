        ...
        <>
            <Card className='compBackgroundExp'>
                <Card.Body className='d-flex flex-column' >
                    ...
                    {/* PARTE DINAMICA DA RICREARE CON LA CHIAMATA API */}
                        {esperienze?.map((esperienze) => (
                        <div className='d-flex justify-content-between flex-column align-items-start expRow'>
                            <img onClick={() => putHandle(esperienze)} className="infoPenExp" src="/src/assets/free_icon.svg" alt=""  />
                            <div className='d-flex justify-content-between align-items-start '>
                                <div className='imgAzienda'>
                                    <img style={{ width: '100%' }} name='companyLogo' src={esperienze?.image}></img>
                                </div>
                                <div className='d-flex flex-column'>
                                    <span className='professione' name='role'>{esperienze?.role}</span>
                                    <span className='azienda' name='company'>{esperienze?.company}</span>
                                    <span className='periodo' name='startDate'>{esperienze?.startDate}</span>
                                    <span className='periodo' name='area'>{esperienze?.area} </span>
                                    <span className='descrizione' name='description'>{esperienze?.description}</span>
                                </div>
                            </div>
                        </div>
            ...
            >
                ...
                <Modal.Body>
                    ...
                    <Button className='float-end' onClick={() => dispatch(postEsperienze(form))}>Salva</Button>
                </Modal.Body>
            </Modal>
